...
- **LoopSummary** — Shown when the user completes all questions in a loop (any scope). Displays correct/incorrect counts, accuracy percentage, and a progress bar. Shows contextual navigation buttons based on the completed scope: global scope shows one "Next Loop" button; topic scope shows "Next Loop - {topic}" and "Next Loop - All Topics"; lesson scope shows all three levels (lesson, topic, all topics). Current-scope button is styled primary (blue); broader-scope buttons are styled secondary (outlined). Accepts props: `loopProgress`, `onNextLoop(scope)`, `scope`, `topicName`, `lessonTitle`.
